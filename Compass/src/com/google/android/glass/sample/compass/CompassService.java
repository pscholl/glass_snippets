...
public class CompassService extends Service {
    ...
    public class CompassBinder extends Binder {
        ...
        protected static final float DEFAULT_HEADING = 255;
        ...
        public void markCurrentHeading() {
          SharedPreferences p = getSharedPreferences(LANDMARKS, 
              MODE_MULTI_PROCESS|MODE_WORLD_WRITEABLE|MODE_WORLD_READABLE);
          float from = p.getFloat(LANDMARK_FROM, DEFAULT_HEADING),
                  to = p.getFloat(LANDMARK_TO, DEFAULT_HEADING),
                 cur = (float) (mOrientationManager.getHeading() * Math.PI/180.F - Math.PI/180.F);
          
          if (from == DEFAULT_HEADING) {
            from = cur;
          } else if (to == DEFAULT_HEADING) {
            to = cur;
          } else {
            if (Math.abs(MathUtils.radbox(cur-from)) < Math.abs(MathUtils.radbox(cur-to)))
              from = cur;
            else
              to = cur;
          }
          ...
          Editor e = p.edit();
          e.putFloat(LANDMARK_FROM, from);
          e.putFloat(LANDMARK_TO, to);
          e.apply();
          
          mRenderer.mCompassView.setMarker(from, to);
        }

        public void resetLandmarks() {
          Editor e = getSharedPreferences(LANDMARKS, 
              MODE_MULTI_PROCESS|MODE_WORLD_WRITEABLE|MODE_WORLD_READABLE).edit();
          
          e.remove(LANDMARK_FROM);
          e.remove(LANDMARK_TO);
          e.apply();
          
          mRenderer.mCompassView.unsetMarker();
        }
    }
